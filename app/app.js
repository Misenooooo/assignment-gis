...
let q = "WITH presov as (SELECT sum(data.poc_tp) AS sum, data.n_u AS title FROM data GROUP BY data.n_u) Select MAX(p.sum) as sum, p.title as title, st_asgeojson(ST_UNION(way)) as geom from presov p join planet_osm_line pol on p.title=pol.name where pol.highway NOT IN ('service', 'track') group by title";
